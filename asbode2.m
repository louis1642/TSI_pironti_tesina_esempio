function [mod,fase,omega]=asbode2(num,den,W_axes,M_axes,F_axes,B,R,X,Y)
...
%     - Y = 0 : traccia sia i diagrammi asintotici dei singoli
...
%     - Y = 3 : (default) traccia entrambi i diagrammi complessivi
...
%     Si veda anche BODE.
%
% Autore: Alessandro Giua. Versione 4.2 (revisionata da Luigi Catello),
% Maggio 2021
ni=nargin;
no=nargout;
...
m=length(z);
n=length(p);
% nu=0;     % non è necessario istanziare la var
A=[b0/a0 0 1];
...
gain = zeros(length(omega),ma);
phase = gain;                           % prealloco le variabili
...
if ((no==0) && (Y==0))
    ...
    hold off;
    if (X==0)
        title('Diagramma di Bode','FontSize',12)        
    else
        title('Diagramma di Bode (modulo)','FontSize',12)
        xlabel('Pulsazione \omega [rad/s]','FontSize',12)
    end
    ylabel('Modulo M [db]','FontSize',12)
    if isempty(M_axes)
    ...
    %
    hold off;
    set(gca,'YTick',-720:45:720,'FontSize',12)
    grid
    if (X==1)
        title('Diagramma di Bode (fase)','FontSize',12)        
    end
    xlabel('Pulsazione \omega [rad/s]','FontSize',12)
    ylabel('Fase \phi [gradi]','FontSize',12)
    if isempty(F_axes)
...
if ((no==0) && (Y~=0)) % TRACCIO SOLO I GRAFICI SPECIFICATI DAL PARAMETRO par
    ...
    switch Y
          ...
          case {2}
            ...
            if (X==0)
                title('Diagramma di Bode asintotico','FontSize',12)
            else
                xlabel('Pulsazione \omega [rad/s]','FontSize',12)
                title('Diagramma di Bode asintotico (modulo)','FontSize',12)
            end  
          case {3}
            semilogx(omega,gain(:,1),'k--',omega,gain(:,2),'b','LineWidth',2);
            if (X==0)
                title('Diagramma di Bode reale e asintotico','FontSize',12)
            else
                xlabel('Pulsazione \omega [rad/s]','FontSize',12)
                title('Diagramma di Bode asintotico (modulo)','FontSize',12)
            end  
    ...
    switch Y
          ...
          case {2}
            ...
            if (X==1)
                title('Diagramma di Bode asintotico (fase)','FontSize',12)
            end
          case {3}
            semilogx(omega,phase(:,1),'k--',omega,phase(:,2),'b','LineWidth',2);
            if (X==1)
                title('Diagramma di Bode reale e asintotico (fase)','FontSize',12)
            end
    end   
    set(gca,'YTick',-720:45:720,'FontSize',12)
    grid
